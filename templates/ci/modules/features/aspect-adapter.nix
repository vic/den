{ denTest, lib, ... }:
{
  ...
  den.fxPipeline = false;
        ...
        den.aspects.foo.meta.adapter = den.lib.aspects.adapters.filter (a: a.name != "baz");
  ...
  den.fxPipeline = false;
        ...
        den.aspects.foo.meta.adapter = den.lib.aspects.adapters.filter (a: a.name != "bar");
        den.aspects.bar.nixos = { };
        den.aspects.baz.nixos = { };
        ...
        expr =
          let
            ...
            outer = trace.over (adapters.filter (a: a.name != "baz"));
          in
          outer "nixos" den.aspects.foo;
