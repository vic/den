import ./_den_aspect.nix (
  ...
  }:
  {
    ...
    provides = {
      host = { root }: { host }: den.import-tree { root = "${toString root}/${host.name}"; };
      user =
        { root }: { host, user }: den.import-tree { root = "${toString root}/${user.name}@${host.name}"; };
      home = { root }: { home }: den.import-tree { root = "${toString root}/${home.name}"; };
    };
  }
)
