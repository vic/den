...
}:
{
  den.provides.import-tree.description = ''
    ...
    usage:
      ...
      this aspect can be included explicitly on any aspect:
          ...
          den.aspects.my_host.includes = [ (den._.import-tree { root = ./.; }) ];

      or it can be default imported per host/user/home:

          
          # each host will import-tree from ./hosts/''${host.name}/_{nixos,darwin}/*.nix
          den.default.host._.host.includes = [ (den._.import-tree._.host { root = ./hosts; }) ];

          # each user will import-tree from ./users/''${user.name}@''${host.name}/_homeManager/*.nix
          den.default.user._.user.includes = [ (den._.import-tree._.user { root = ./users; }) ];

          # each home will import-tree from ./homes/''${home.name}/_homeManager/*.nix
          den.default.home._.home.includes = [ (den._.import-tree._.home { root = ./homes; }) ];
  ...
  den._.import-tree.provides = {
    host = { root }: { host }: den._.import-tree { root = "${toString root}/${host.name}"; };
    user =
      { root }:
      { host, user }:
      den._.import-tree { root = "${toString root}/${user.name}@${host.name}"; };
    home = { root }: { home }: den._.import-tree { root = "${toString root}/${home.name}"; };
  };
}
