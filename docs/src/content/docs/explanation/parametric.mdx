...
[`nix/lib.nix`](https://github.com/vic/den/blob/main/nix/lib.nix) · [`nix/fn-can-take.nix`](https://github.com/vic/den/blob/main/nix/fn-can-take.nix)
</Aside>
...
<Aside >
Aspects created by Den from your host/user/home definitions are already `parametric`.
They will forward context to their includes functions.

However for new aspects you define manually, they are not parametric unless you say so.
...
It likely means you are trying to forward context from an aspect that is not `parametric`.
See also [#182](https://github.com/vic/den/discussions/182)
</Aside>
...
For individual functions (not whole aspects), use [`den.lib.take`](/reference/lib/#denlibtake):
