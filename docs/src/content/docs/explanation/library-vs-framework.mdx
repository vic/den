...
Den's core (`nix/lib.nix`) is domain-agnostic. It provides:
...
| `aspects` | The flake-aspects API (resolve, merge, types) |
...
{ my-aspects, den, ... }: {
  ...
  # create a private isolated aspect namespace independent of den.aspects
  imports = [ (den.namespace "my-aspects" false) ];
  ...
  aspect = my-aspects.web-server { env = "production"; };
