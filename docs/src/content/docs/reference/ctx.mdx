...
| `into.hm-host` | Detect HM support (from `hm-os.nix`) |
| `into.maid-host` | Detect nix-maid support (from `maid-os.nix`) |
| `into.hjem-host` | Detect hjem support (from `hjem-os.nix`) |
...
2. At least one user has `"homeManager"` in their `classes`
...
| Field | Value |
|-------|-------|
| `description` | Host with nix-maid users |
| `provides.maid-host` | `{ host }:` imports nix-maid NixOS module |
| `into.maid-user` | Enumerate maid-class users |

**Detection criteria** (all must be true):
1. Host class is `nixos` or `darwin`
2. At least one user has `"maid"` in their `classes`
3. `inputs.nix-maid` exists, or host has a custom `maid-module` attribute

### den.ctx.maid-user

| Field | Value |
|-------|-------|
| `provides.maid-user` | `{ host, user }:` forward maid class to `users.users.<name>.maid` |

### den.ctx.hjem-host

| Field | Value |
|-------|-------|
| `description` | Host with hjem users |
| `provides.hjem-host` | `{ host }:` imports hjem NixOS module |
| `into.hjem-user` | Enumerate hjem-class users |

**Detection criteria** (all must be true):
1. Host class is `nixos` or `darwin`
2. At least one user has `"hjem"` in their `classes`
3. `inputs.hjem` exists, or host has a custom `hjem-module` attribute

### den.ctx.hjem-user

| Field | Value |
|-------|-------|
| `provides.hjem-user` | `{ host, user }:` forward hjem class to `hjem.users.<name>` |
