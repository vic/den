...
<Aside type="caution">
Angle brackets use Nix's `__findFile` mechanism, which is positional
(depends on being in scope). They only work in modules where
`__findFile` has been set as a module argument.
</Aside>
