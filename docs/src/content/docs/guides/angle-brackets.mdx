...
<Aside title="Use the Source, Luke" icon="github">[`nix/den-brackets.nix`](https://github.com/vic/den/blob/main/nix/den-brackets.nix)</Aside>
...
```nix
{ __findFile, ... }: {
  ns.moo.silly = true; # write access
  expr = <ns/moo>;  # read resolves to den.ful.ns.moo
}
```
