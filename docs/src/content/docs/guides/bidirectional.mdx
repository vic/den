...
Crucial points here are when `den.aspects.igloo.includes` are called with `{ host }` and later with `{ host, user }` per-user.

Because the list of aspects at `igloo.includes` get invoked more than once, with different contexts,
