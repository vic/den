...
<Aside title="When is bidirectionality needed?" icon="comment-alt">
Only when you wish for a Host-aspect to provide common _home settings_ 
(`homeManager`, etc) for **all** its users.

And even then there are much simpler and better ways:

```nix
# an aspect with all common stuff for everybody
den.aspects.user-base = { nixos = ...; homeManager = ...; hjem = ...; };

# include it in all users
den.ctx.user.includes = [ den.aspects.user-base ];
```

> _Now you can skip the rest of this page_.
</Aside>


There are at least two (built-in) ways to achieve this in Den.
The `den._.bidirectional` and `den._.mutual-provider` batteries. The
first one was extracted from den-core and the second started
life as an **aspect routing** example, but made it to become
proper battery itself and even safer than the first.
...
<Aside title="Avoid dups with host-only module pattern" icon="rocket">
Here's a pattern you can use to avoid duplication while `bidirectional` is enabled:

```nix
den.aspects.igloo.includes = [ 
  # wrap an aspect with perHost
  (den.lib.perHost igloo-only) 

  # any other aspect can contribute to users
  shells
  music-production
];

# Use this aspect for host-only stuff, anything that would
# lead to duplicates is safe in this list
den.aspects.igloo-only = {
  includes = with den.aspects; [ facter disko ];
  nixos = { pkgs, ... }: {
