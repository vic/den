...
Be sure to read about the entity [Schemas](/reference/schema/), each of `host`/`user`/`home` has a corresponding `den.base.*` module for meta-configuration. Which can later be used in aspects that read from `host`. These schemas are the meta-data equivalent of what Dendritic flake-parts users do with flake-level options.
</Aside>
...
| `*` | from `den.base.host` | Any option defined by base module |
...
| `*` | from `den.base.user` | Any option defined by base module |
| `*` |  | Any other free-form attribute |
...
`den.base.{host,user,home,conf}` provides shared configuration applied to all entities of each kind.
...
Each host and user has an `aspect` attribute name (defaults to the config `name`).
Den looks up `den.aspects.${aspect}` to find the configuration for that entity:

```nix
{
  den.hosts.x86_64-linux.laptop.users.alice = { };
  # Den uses den.aspects.laptop for the host
  # Den uses den.aspects.alice for the user
}
```
