...
This creates NixOS/Darwin configurations with users sharing the same home configuration, ready for:
...
den.hosts.x86_64-linux.my-laptop = {
  ...
  intoAttr = [ "nixosConfigurations" "yavanna" ]; # default: [ "nixosConfigurations" "my-laptop" ]
  users.vic = {
    userName = "vborja";       # default: vic
    aspect = "oeiuwq";         # default: vic
    classes = ["homeManager"]; # default: ["homeManager"]
  };
};
```
...
All three machines share same `den.aspects.vic` user aspect.
...
den.hosts.x86_64-linux.my-laptop = {
  ...
  location = "studio";    # custom attribute
  users.vic = { };
};
```
...
Override where configurations are placed:

```nix
den.hosts.x86_64-linux.wsl-box = {
  intoAttr = [ "wslConfigurations" "my-wsl" ];
  wsl.enable = true;
  users.wsl = { };
};
```

Custom nixpkgs instance, home-manager osConfig and output attribute:

```nix
den.homes.x86_64-linux.tux-at-igloo = {
  aspect = "tux"; # Use den.aspects.tux for configuration
  intoAttr = [ "homeConfigurations" "tux@igloo" ];
  instantiate = { pkgs, modules }:
    inputs.home-manager-stable.lib.homeManagerConfiguration {
      inherit modules;
      pkgs = inputs.self.nixpkgs-stable.legacyPackages.x86_64-linux;
      extraSpecialArgs.osConfig = inputs.self.nixosConfigurations.igloo.config;
    };
};
```
